...
@torch.no_grad()
def evaluate_memory_bpc():
    bpc_dict = {}

    model.eval()
    
    for split in ['val', 'test']:
        dataset = enwikIterator(split, block_size * 9, device_type) # 9 chunks since we can condition on up to 8 past memories
        ds = iter(dataset)

        bpcs = torch.zeros(dataset.len())

        for seq in ds:
            bpc = eval_on_persistent_memory(seq, model)
            bpcs[ds.idx-1] = bpc
